...
def bookmark():
    ...
    # return redirect(url_for("main.index", recipes=recipes, bookmarks=bookmarks, sort=sort_op, category=category, search=search, login=False, signup=False))
...
@bp.route("/bookmark_profile", methods=["POST"])
@flask_login.login_required
def bookmark_profile():
    recipe_id = request.form.get("recipe-id")
    profile_user_id = request.form.get("profile-user-id")
    user_id = flask_login.current_user.id
    query = db.select(model.Bookmark).where(model.Bookmark.recipe_id == recipe_id).where(model.Bookmark.user_id == user_id)
    bookmark = db.session.execute(query).scalar_one_or_none()
    if bookmark:
    ...
    return redirect(url_for("profile.get_profile", user_id = profile_user_id))
